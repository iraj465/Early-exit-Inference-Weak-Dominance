...
def train_backbone(model, train_dl, valid_dl, save_path, epochs=50,
    ...
    for epoch in range(epochs):
        ...
        for xb, yb in train_dl:
            results = model(xb)
            print("BACKBONE HHUHUHUH")
            print("\nTrue\n")
            print(yb)
            print("*******************************")
            print("Pred\n")
            pred = torch.argmax(results[-1],axis=1)
            print(pred)
...
def train_joint(model, train_dl, valid_dl, save_path, opt=None,
    ...
    results_list = []
    results_list.append(["Epoch","True Label","Predicted Label(Layer 0)","Predicted Label(Layer 1)","Predicted Label(Layer 2)"])
    for epoch in range(joint_epochs):
        ...
        #training loop
        for xb, yb in train_dl:
            results,list_exits = model(xb,True)
            # pred = torch.argmax(results[-1],axis=1)
            # print("|TRUE ",yb,"|Exit 1",list_exits[0],"|Exit 2",list_exits[1],"|Final",pred)
            # print("*******************************")
            # results_list.append([epoch+1,yb.item(),list_exits[0].item(),list_exits[1].item(),pred.item()])
        ...
        #Generate CSV file
        # with open('uss.csv', 'w', newline='') as file:
...
def main():
    ...
    jt_epochs = 1 #100 for joint with exits
    path_str = 'outputs/'
