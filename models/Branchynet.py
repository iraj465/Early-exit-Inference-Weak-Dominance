...
class Branchynet(nn.Module):
    ...
    def _forward_training(self, x,sendExits=False):
        ...
        for bb, ee in zip(self.backbone, self.exits):
            x = bb(x)
            res.append(ee(x))
        ex1 = torch.argmax(res[0],axis=1)
        ex2 = torch.argmax(res[1],axis=1)
        list_exits = [ex1,ex2]
        if sendExits:
            return res,list_exits
        else:
            return res

    def forward(self, x,sendExits=False):
        ...
        if self.fast_inference_mode:
            ...
            #calculate all exits
            return self._forward_training(x,sendExits)
